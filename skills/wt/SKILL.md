...
3. Run the tesser canonical workflow, with every tesser command cwd'd into
   the worktree — `(cd "$WT" && tesser ...)` — since it syncs the *current*
...
4. Report branch, worktree path, workspace id, box id, and tell the user to
   open http://localhost:3000. Work per the tesser skill from then on.
